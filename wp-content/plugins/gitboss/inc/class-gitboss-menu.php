...
class Gitium_Menu {
	...
	public $gitium_menu_slug       = 'gitboss/gitboss.php';
	public $commits_menu_slug      = 'gitboss/gitboss-commits.php';
	public $settings_menu_slug     = 'gitboss/gitboss-settings.php';
	public $requirements_menu_slug = 'gitboss/gitboss-requirements.php';
	public $git = null;
	...
	protected function show_disconnect_repository_button() {
		...
		<form name="gitium_form_disconnect" id="gitium_form_disconnect" action="" method="POST">
		  ...
		  ?>
			<input type="submit" name="GitiumSubmitDisconnectRepository" value='<?php _e( 'Disconnect from repo', 'gitboss' ); ?>' class="button secondary" onclick="return confirm('<?php _e( 'Are you sure you want to disconnect from the remote repository?', 'gitboss' ); ?>')"/>&nbsp;
		</form>
		<?php
	}
}

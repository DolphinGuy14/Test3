...
class Gitium_Submenu_Commits extends Gitium_Menu {
	...
	public function table_head() {
		...
		<thead>
		<tr>
			<th scope="col"><?php _e( 'Commits', 'gitboss' ); ?></th>
			<th scope="col"></th>
		</tr>
		</thead>
		<?php
	...
	public function page() {
		?>
		<div class="wrap">
		<h2><?php printf( __( 'Last %s commits', 'gitboss' ), GITIUM_LAST_COMMITS ); ?></h2>
		...
		foreach ( $this->git->get_last_commits( GITIUM_LAST_COMMITS ) as $commit_id => $data ) {
			...
			if ( isset( $committer_name ) ) {
				$committer         = "<span title='$committer_email'> -> $committer_name " . sprintf( __( 'committed %s ago', 'gitboss' ), human_time_diff( strtotime( $committer_date ) ) ) . '</span>';
			...
			<td style="position:relative">
				...
				<span title="<?php echo esc_attr( $author_email ); ?>"><?php echo esc_html( $author_name ) . ' ' . sprintf( __( 'authored %s ago', 'gitboss' ), human_time_diff( strtotime( $author_date ) ) ); ?></span><?php echo $committer; ?></div>
			</td>

...
class Gitium_Submenu_Configure extends Gitium_Menu {
	...
	public function regenerate_keypair() {
		...
		$this->success_redirect( __( 'Keypair successfully regenerated.', 'gitboss' ) );
	}
	...
	public function init_process( $remote_url ) {
		...
		if ( count( $git->get_remote_branches() ) == 0 ) {
			...
			$git->commit( __( 'Initial commit', 'gitboss' ), $current_user->display_name, $current_user->user_email );
			if ( ! $git->push( 'master' ) ) {
				$git->cleanup();
				return false;
			}
	...
	public function init_repo() {
		...
		if ( empty( $_POST['remote_url'] ) ) {
			$this->redirect( __( 'Please specify a valid repo.', 'gitboss' ) );
		}
		if ( $this->init_process( $_POST['remote_url'] ) ) {
			$this->success_redirect();
		} else {
			$this->redirect( __( 'Could not push to remote', 'gitboss' ) . ' ' . $_POST['remote_url'] );
		}
	}
	...
	public function choose_branch() {
		...
		$commit = $this->git->commit( __( 'Merged existing code from ', 'gitboss' ) . get_home_url(), $current_user->display_name, $current_user->user_email );
		if ( ! $commit ) {
			$this->git->cleanup();
			$this->redirect( __( 'Could not create initial commit -> ', 'gitboss' ) . $this->git->get_last_error() );
		}
		if ( ! $this->git->merge_initial_commit( $commit, $branch ) ) {
			$this->git->cleanup();
			$this->redirect( __( 'Could not merge the initial commit -> ', 'gitboss' ) . $this->git->get_last_error() );
		}
	...
	private function setup_step_1_remote_url() {
		?>
		<tr>
		<th scope="row"><label for="remote_url"><?php _e( 'Remote URL', 'gitboss' ); ?></label></th>
			<td>
				<input type="text" class="regular-text" name="remote_url" id="remote_url" placeholder="git@github.com:user/example.git" value="">
				<p class="description"><?php _e( 'This URL provide access to a Git repository via SSH, HTTPS, or Subversion.', 'gitboss' ); ?><br />
		<?php _e( 'If you need to authenticate over "https://" instead of SSH use: <code>https://user:pass@github.com/user/example.git</code>', 'gitboss' ); ?></p>
			</td>
		</tr>
		<?php
	}
	...
	private function setup_step_1_key_pair() {
		if ( ! defined( 'GIT_KEY_FILE' ) || GIT_KEY_FILE == '' ) :
			...
			<tr>
			<th scope="row"><label for="key_pair"><?php _e( 'Key pair', 'gitboss' ); ?></label></th>
				<td>
					<p>
					<input type="text" class="regular-text" name="key_pair" id="key_pair" value="<?php echo esc_attr( $git_public_key ); ?>" readonly="readonly">
					<input type="submit" name="GitiumSubmitRegenerateKeypair" class="button" value="<?php _e( 'Regenerate Key', 'gitboss' ); ?>" />
					</p>
					<p class="description"><?php _e( 'If your code use ssh keybased authentication for git you need to allow write access to your repository using this key.', 'gitboss' ); ?><br />
			<?php _e( 'Checkout instructions for <a href="https://help.github.com/articles/generating-ssh-keys#step-3-add-your-ssh-key-to-github" target="_blank">github</a> or <a href="https://confluence.atlassian.com/display/BITBUCKET/Add+an+SSH+key+to+an+account#AddanSSHkeytoanaccount-HowtoaddakeyusingSSHforOSXorLinux" target="_blank">bitbucket</a>.', 'gitboss' ); ?>
					</p>
				</td>
			</tr>
		<?php endif;
	...
	private function setup_step_1() {
		?>
		<div class="wrap">
			<h2><?php _e( 'Configuration step 1', 'gitboss' ); ?></h2>
			...
			<form action="" method="POST">
				...
				<p class="submit">
				<input type="submit" name="GitiumSubmitFetch" class="button-primary" value="<?php _e( 'Fetch', 'gitboss' ); ?>" />
				</p>
			</form>
		</div>
		<?php
	...
	private function setup_step_2() {
		$git = $this->git; ?>
		<div class="wrap">
		<h2><?php _e( 'Configuration step 2', 'gitboss' ); ?></h2>
		<p><?php _e( 'If you need help to set this up, please click on the "Help" button from the top right corner of this screen.' ); ?></p>
		...
		<table class="form-table">
		<tr>
		<th scope="row"><label for="tracking_branch"><?php _e( 'Choose tracking branch', 'gitboss' ); ?></label></th>
			<td>
				...
				<p class="description"><?php _e( 'Your code origin is set to', 'gitboss' ); ?> <code><?php echo esc_html( $git->get_remote_url() ); ?></code></p>
			</td>
		</tr>
		</table>

		<p class="submit">
		<input type="submit" name="GitiumSubmitMergeAndPush" class="button-primary" value="<?php _e( 'Merge & Push', 'gitboss' ); ?>" />
		</p>
		</form>
		<?php

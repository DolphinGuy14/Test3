...
function gitium_check_after_event( $plugin, $event = 'activation' ) {
	...
	if ( 'gitboss/gitboss.php' == $plugin ) { return; } // do not hook on activation of this plugin
...
function _gitium_generate_keypair() {
	...
	$public_key = 'ssh-rsa ' . base64_encode( $buffer ) . ' gitboss@' . parse_url( get_home_url(), PHP_URL_HOST );
...
function gitium_get_webhook() {
	...
	$url = add_query_arg( 'key', $key, plugins_url( 'gitboss-webhook.php', __FILE__ ) );

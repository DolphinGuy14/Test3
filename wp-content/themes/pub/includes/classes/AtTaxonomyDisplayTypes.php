...
class AtTaxonomyDisplayTypes extends TdTaxonomyFieldManager
{
	...
	public function hookAddFormInsertField() {
		...
		foreach ( $this->getSelectOptions() as $value => $label ) {
			$selectOptions .= '<option value="' . esc_attr( $value ) . '" label="' . esc_attr( $label ) . '"></option>';
		}
	...
	public function hookEditFormInsertField( $term ) {
		...
		foreach ( $this->getSelectOptions() as $value => $label ) {
			$selected = '';
			if ( $value == $taxonomyDisplayType ) {
				$selected = 'selected="selected"';
			}
			$selectOptions .= '<option ' . esc_attr( $selected ) . ' value="' . esc_attr( $value ) . '" label="' . esc_attr( $label ) . '"></option>';
		}
